...
/**
 * This class manages the communication with the user through a GUI. It opens a
 * GuiMainView and transfers all settings and actions to the Model.
 */
public class GuiMainController implements Controller, ActionListener {
	...
	/**
	 * Reads out the configuration in the View and stores it in the Model.
	 */
	...
	/**
	 * Displays a PasswordDialog one or more times where the user can type in a
	 * password, in case the input was both times the same it returns the
	 * password, otherwise it returns null.
	 * 
	 * @param times
	 *          How often the PasswordDialog should be displayed.
	 * @return The typed in password if the passwords matched, otherwise null.
	 */
	...
	public void actionPerformed(final ActionEvent e) {
		...
		if (msg.equals("encrypt")) {
			...
			String pass = model.getPassword() == null ? getPassword(2) : model
			    .getPassword();
			if (pass != null) {
			...
			saveConfigurationToModel();
			String pass = model.getPassword() == null ? getPassword(1) : model
			    .getPassword();
			if (pass != null) {
				model.setPassword(pass);
				model.decrypt();
			...
			int result = JOptionPane.showConfirmDialog(view,
			    bundle.getString("stop_warning_text"),
			    bundle.getString("stop_warning_title"), JOptionPane.YES_NO_OPTION);
			...
			if (algorithm != null) {
				AlgorithmInfoFrame algFrame = new AlgorithmInfoFrame(this, algorithm);
