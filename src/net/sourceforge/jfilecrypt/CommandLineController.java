...
/**
 * This class processes the given command line arguments. It starts an
 * en-/decryption process by itself if there is such an argument. If not this
 * class saves the given arguments in the @see Model and starts the @see
 * GuiMainController which can read out the settings from the Model. On this way
 * you can totally control jFileCrypt from the command line or make some just
 * make some presetting for the GUI.
 */
public class CommandLineController implements Controller {
	...
	public CommandLineController(Model model, String[] args) {
		...
		if (opt.check("lstalg")) {
			...
			if (encset.isSet("o")) {
				model
				    .setTargetArchive(new File(encset.getOption("o").getResultValue(0)));
			}
			if (encset.isSet("c")) {
				...
				try {
					...
					displayError(bundle.getString("wrong_compression_level"), "\"" + lvl
					    + "\"");
			...
			if (decset.isSet("o")) {
				model.setTargetDirectory(new File(decset.getOption("o").getResultValue(
				    0)));
			}
			...
			/*
			 * the GUI controller should catch the default settings from the Model by
			 * himself
			 */
	 ...
	 * 
	 * It is a mess that the password is displayed in plain text. But the only
	 * "nice" solution is coming not until Java 1.6.
	 */
	...
	private void printAlgorithmInfo(String name) {
		...
		for (int i = 0; i < alg.length; i++) {
			if (alg[i].getName().equalsIgnoreCase(name)) {
				...
				if (pwLen <= 0) {
					sysOut.println(bundle.getString("key_length") + ": "
					    + bundle.getString("any_password_length"));
				} else {
					sysOut.println(bundle.getString("key_length") + ": " + pwLen);
				}
				sysOut.println(bundle.getString("author") + ": " + alg[i].getAuthor());
				sysOut
				    .println(bundle.getString("license") + ": " + alg[i].getLicense());
				sysOut
				    .println(bundle.getString("website") + ": " + alg[i].getWebsite());
				sysOut.println("");
				return;
			}
		}
	...
	/**
	 * Displays an error message. The title is needless on the command line.
	 */
	...
	public int shallOverwriteFile(File file) {
		...
		while (true) {
			try {
				...
				sysOut.print(bundle.getString("overwrite_this_file") + " "
				    + file.getCanonicalPath() + "? : [n] ");
				String input = br.readLine();
