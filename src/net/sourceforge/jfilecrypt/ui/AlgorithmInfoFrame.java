...
public class AlgorithmInfoFrame extends JFrameExtended {
	...
	public AlgorithmInfoFrame(Controller controller, Algorithm algorithm) {
		...
		BorderPanel bpAlgorithm = new BorderPanel(bundle.getString("algorithm"));
		bpAlgorithm.add(new JLabel(algorithm.getName()));

		BorderPanel bpPassphraseLength = new BorderPanel(
		    bundle.getString("key_length"));
		String passLengthText = "";
		if (algorithm.getKeyLength() <= 0)
			passLengthText = bundle.getString("any_password_length");
		else
			passLengthText = "" + algorithm.getKeyLength() + " "
			    + bundle.getString("password_will_shorted");
		bpPassphraseLength.add(new JLabel(passLengthText));

		BorderPanel bpAuthor = new BorderPanel(bundle.getString("author"));
		bpAuthor.add(new JLabel(algorithm.getAuthor()));

		BorderPanel bpLicense = new BorderPanel(bundle.getString("license"));
		bpLicense.add(new JLabel(algorithm.getLicense()));

		BorderPanel bpWebsite = new BorderPanel(bundle.getString("website"));
