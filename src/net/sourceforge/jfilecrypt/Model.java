...
public class Model {
	...
	private byte cmpLevel = 0; // defaut compression level = store only
	private FileList fileList = new FileList(""); // files to en-/decrypt
	private String password = null; // password for en-/decrypt
	private String algorithmName = "Blowfish"; // default algorithm for
	                                           // en-/decrypt
	private File targetArchive = null, targetDirectory = null; // the archive file
	                                                           // when encrypting,
	                                                           // the output
	                                                           // directory when
	                                                           // decrypting
	 ...
	 * if you want to get more information about a algorithm which is saved in the
	 * Algorithm class.
	 */
	...
	/**
	 * Returns the current compression level, does not matter if use Zip is set or
	 * not.
	 */
	...
	/**
	 * Returns the algorithm which should be used for en-/decryption.
	 */
	...
	public File getTargetArchive() {
		if (targetArchive == null) {
			...
			if (abspath0.lastIndexOf('.') > 0) {
				def = new File(abspath0.substring(0, abspath0.lastIndexOf('.'))
				    + currentAlgorithm.getSuffix());
			} else {
	...
	public File getTargetDirectory() {
		if (targetDirectory == null) {
			String abspath0 = fileList.get(0).getAbsolutePath();
			targetDirectory = Application.getController().askForTargetDirectory(
			    new File(abspath0.substring(0, abspath0.length()
			        - currentAlgorithm.getSuffix().length())));
		}
	...
	private void updateProgress() {
		...
		// Application.getController().displayVerbose("Progress calculated: " +
		// dprog);
			...
			public void run() {
				...
				if (!alg.initEncrypt(password)) {
					Application.getController().displayError(
					    bundle.getString("enc_init_fail_title"),
					    bundle.getString("enc_init_fail_text"));
					return;
				}
				...
				try {
					...
					} else if (EncryptionMode.getBestEncryptionMode(alg
					    .getEncryptionMode()) == EncryptionMode.MODE_BLOCK) {
					...
					Application.getController().displayError(
					    bundle.getString("error_file_not_exist"),
					    ex.getLocalizedMessage());
				} catch (IOException ex) {
					Application.getController().displayError(
					    bundle.getString("error_generic_io"), ex.getLocalizedMessage());
				}
			}
		};
		runningThread.start();
	...
	private boolean mayOverwrite(File file) {
		...
		if (overwriteMode == Controller.OVERWRITE_ALL_YES
		    || overwriteMode == Controller.OVERWRITE_ONCE_YES) {
			return true;
		} else {
			return false;
		}
	...
	private void copyFileToArchive(final ZipArchiveOutputStream zos,
	    final File file, final String dir) {
		...
		try {
			if (file.isFile()) {
				...
				Application.getController().displayVerbose(
				    "Hash of " + zae.getName() + ": "
				        + new Base64().encodeToString(md.digest()));
			...
			Application.getController().displayError(
			    bundle.getString("unknown_alg_text"), ex.getLocalizedMessage());
		} finally {
	...
	/**
	 * Implements the decrypt procedure, consisting of determining the used
	 * algorithm, opening the archive, reading the stored files and decrypting
	 * them.
	 */
		...
		runningThread = new Thread() {
			...
			public void run() {
				...
				for (int f = 0; f < fileList.size(); f++) {
					...
					String arsuf = arname.substring(arname.lastIndexOf('.'),
					    arname.length());
					...
					if (alg == null) {
						Application.getController().displayError(
						    bundle.getString("unknown_alg_title"),
					...
					if (!alg.initDecrypt(password)) {
						Application.getController().displayError(
						    bundle.getString("dec_init_fail_title"),
					...
					try {
						...
						File outputDir = getTargetDirectory();// new
						                                      // File(archive.getAbsolutePath().substring(0,
						                                      // archive.getAbsolutePath().length()-alg.getSuffix().length())+"-decrypted/");
						...
						if (!outputDir.exists()) {
							if (!outputDir.mkdir()) {
								Application.getController().displayError(
								    bundle.getString("output_dir_fail_title"),
								    outputDir.getAbsolutePath() + " "
								        + bundle.getString("output_dir_fail_text"));
								return;
							}
						}
						...
						while ((zae = zis.getNextZipEntry()) != null) {
							...
							Application.getController().displayVerbose(
							    "writing to file: " + out.getAbsolutePath());
							...
							Application.getController().displayVerbose(
							    "Length of zip entry " + zae.getName() + " is " + length
							        + "b");
							...
							if (zae.getComment() != null && zae.getComment().length() > 0) {
								if (Arrays.equals(md.digest(),
								    new Base64().decode(zae.getComment()))) {
									Application.getController().displayVerbose(
									    "Hash of " + zae.getName() + ": "
									        + new Base64().encodeToString(md.digest()));
									Application
									...
									Application.getController().displayVerbose(
									    "the hash of " + zae.getName()
									        + " was verified succesfully");
								}
							}
						}
						if (!gotEntries) {
							Application.getController().displayError(
							    bundle.getString("error_no_entries_title"),
	...
	public String toString() {
		...
		rtr += "filenames: \n";
		Iterator<File> it = fileList.getIterator();
		while (it.hasNext()) {
			File f = it.next();
			rtr += f.getPath() + "\n";
		}
