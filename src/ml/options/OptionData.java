 ...
 * key, the separator (for value options), the multiplicity, and all the other
 * settings describing the option. The class is designed to be only a data
 * container from a user perspective, i. e. the user has read-access to any data
 * determined by the {@link Options#check()}, but not access to any of the other
 * methods which are used internally for the operation of the actual check.
 */
...
public class OptionData {
	 ...
	 * options, a non-<code>null</code> value will be returned. Non-value options
	 ...
	 * {@link #getResultCount()}<code> - 1</code>. However, only for value options
	 * which take details, a non-<code>null</code> detail will be returned.
	 * Non-value options and value options which do not take details always return
	 * <code>null</code>.
	 * <p>
	 * 
	 * @param index
