...
public class Options {
	...
	public enum Separator {
		...
		/**
		 * This is just a placeholder in case no separator is required (i. e. for
		 * non-value options)
		 */

		NONE('D'); // NONE is a placeholder in case no separator is required, 'D' is
							 // just an arbitrary dummy value
	 ...
	 * @param prefix
	 *          The prefix to use for all command line options. It can only be set
	 *          here for all options at the same time
	 * @param defaultMultiplicity
	 *          The default multiplicity to use for all options (can be overridden
	 ...
	 *           <code>defaultMultiplicity</code> is <code>null</code> - or if the
	 *           data range values don't make sense
	 */
	 ...
	 *          when adding an option)
	 * @param data
	 *          The default minimum and maximum number of data items for all sets
	 *          (can be overridden when adding a set)
	 *          <p>
	 * @throws IllegalArgumentException
	 *           If either <code>args</code>, <code>prefix</code>, or
	 *           <code>defaultMultiplicity</code> is <code>null</code> - or if the
	 *           data range value doesn't make sense
	 */
	 ...
	 *          <p>
	 * @throws IllegalArgumentException
	 *           If either <code>args</code> or <code>defaultMultiplicity</code>
	 *           is <code>null</code> - or if the data range values don't make
	 *           sense
	 */
	 ...
	 * @throws IllegalArgumentException
	 *           If either <code>args</code> or <code>defaultMultiplicity</code>
	 *           is <code>null</code> - or if the data range value doesn't make
	 *           sense
	 */
	...
	/**
	 * Constructor. The prefix is set to {@link Prefix#DASH}, and the multiplicity
	 * is set to {@link Multiplicity#ONCE}.
	 * <p>
	 * 
	 * @param args
	 *          The command line arguments to check
	 ...
	 *          <p>
	 * @throws IllegalArgumentException
	 *           If <code>args</code> is <code>null</code> - or if the data range
	 ...
	 * @throws IllegalArgumentException
	 *           If either <code>args</code> or <code>prefix</code> is
	 *           <code>null</code> - or if the data range values don't make sense
	 */
	...
	/**
	 * Return the (first) matching set. This invocation does not ignore unmatched
	 * options and requires that data items are the last ones on the command line.
	 * <p>
	 * 
	 * @return The first set which matches (i. e. the <code>check()</code> method
	 *         returns <code>true</code>) - or <code>null</code>, if no set
	 *         matches.
	 */
	 ...
	 *          A boolean to select whether unmatched options can be ignored in
	 *          the checks or not
	 ...
	 *          <p>
	 * @return The new <code>Optionset</code> instance created. This is useful to
	 *         allow chaining of <code>addOption()</code> calls right after this
	 *         method
	 */
	...
	public OptionSet addSet(String setName, int minData, int maxData) {
		...
		OptionSet os = new OptionSet(prefix, defaultMultiplicity, setName, minData,
		    maxData);
	...
	/**
	 * Return an option set - or <code>null</code>, if no set with the given name
	 * exists
	 * <p>
	 * 
	 * @param setName
	 ...
	 *          <p>
	 * @return The set to retrieve (or <code>null</code>, if no set with the given
	 *         name exists)
	 */
	 ...
	 * any of the <code>check()</code> methods). This is useful to determine what
	 * was wrong with the command line arguments provided
	 * <p>
	 * 
	...
	/**
	 * Run the checks for the default set. <code>ignoreUnmatched</code> is set to
	 ...
	 *         <p>
	 * @throws IllegalArgumentException
	 *           If either <code>setName</code> is <code>null</code>, or the set
	 *           is unknown.
	 */
	...
	public boolean check(String setName, boolean ignoreUnmatched,
		...
		if (options.size() == 0) { // No options have been defined at all
			if (arguments.length == 0) { // No arguments have been given: in this
		...
		while (true) {
			...
			for (OptionData optionData : options) { // For each argument, we may need
																							// to check all defined options
				...
				if (m.lookingAt()) {
					if (optionData.useValue()) { // The code section for value options
						...
						if (optionData.getSeparator() == Separator.BLANK) { // In this case,
																																// the next
																																// argument must
																																// be the value
							if (ipos + 1 == arguments.length) { // The last argument, thus no
																									// value follows it: Error
								checkErrors
								...
								if (next.startsWith(pre)) { // The next one is an argument, not
																						// a value: Error
		...
		for (int i = 0; i < matched.length; i++) { // Assemble the list of unmatched
																							 // options
			if (!matched[i]) {
		...
		for (OptionData optionData : options) {
			...
			Multiplicity mu = optionData.getMultiplicity();

			if (mu.equals(Multiplicity.ONCE)) {
				if (optionData.getResultCount() != 1)
					err = true;
			} else if (mu.equals(Multiplicity.ONCE_OR_MORE)) {
				if (optionData.getResultCount() == 0)
					err = true;
			} else if (mu.equals(Multiplicity.ZERO_OR_ONE)) {
				if (optionData.getResultCount() > 1)
					err = true;
			}
			/*
			 * switch (optionData.getMultiplicity()) { case ONCE: if
			 * (optionData.getResultCount() != 1) err = true; break; case
			 * ONCE_OR_MORE: if (optionData.getResultCount() == 0) err = true; break;
			 * case ZERO_OR_ONE: if (optionData.getResultCount() > 1) err = true;
			 * break; }
			 */
	...
	public void addOptionAllSets(String key, Multiplicity multiplicity) {
		for (String setName : optionSets.keySet())
			optionSets.get(setName).addOption(key, false, Separator.NONE, false,
			    multiplicity);
	}
	...
	/**
	 * This is the overloaded {@link Object#toString()} method, and it is provided
